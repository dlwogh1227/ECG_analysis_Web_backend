...
//                "gpt_result", gpt_result // gpt 결과
    ...
    @PostMapping("upload-ecgImage-lead2only-mobile")
    public ResponseEntity<?> uploadEcgImageLead2only_mobile(
            @RequestParam("file") MultipartFile ecgImage,
            @RequestParam("questionnaire") String questionnaireJson
    ) throws IOException {
        // Flask 서버의 해당 엔드포인트 주소
        String flaskUrl = "http://localhost:5000/lead2only_mobile";  // Flask에서 정의된 엔드포인트 이름에 맞게 수정
